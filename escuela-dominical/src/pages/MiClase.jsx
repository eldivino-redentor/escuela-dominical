...
export default function MiClase() {
  ...
  // Mantener refs actualizadas para usar en autoSave
  ...
  function updateField(miembroId, field, value) {
    ...
    // Auto-guardar 2 segundos después del último cambio
    if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current)
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-xs font-medium">
          ...
          {saveStatus === ''       && semana && <span className="text-gray-300">Auto-guardado activo</span>}
        </div>
      </div>
        ...
        <>
          ...
          <div className="card mb-4 bg-blue-700 text-white border-0">
            <div className="grid grid-cols-4 gap-2 text-center">
              <div>
                <div className="text-xl font-bold">{presentes}</div>
                <div className="text-xs text-blue-200">Presentes</div>
              </div>
              <div>
                <div className="text-xl font-bold">{totalCapitulos}</div>
                <div className="text-xs text-blue-200">Capítulos</div>
              </div>
              <div>
                <div className="text-xl font-bold">${totalCoop.toFixed(0)}</div>
                <div className="text-xs text-blue-200">Cooperación</div>
              </div>
              <div>
                <div className="text-xl font-bold">${totalMisionero.toFixed(0)}</div>
                <div className="text-xs text-blue-200">Peso Mis.</div>
              </div>
            </div>
          </div>
          ...
          {/* Botón guardar manual (respaldo) */}
          <div className="sticky bottom-20 md:bottom-4">
            <button onClick={guardarAhora} className={`w-full py-3 rounded-xl font-semibold text-base shadow-lg transition-all ${
              saveStatus === 'saved' ? 'bg-green-600 text-white' : 'bg-blue-700 text-white hover:bg-blue-800'
            }`}>
              {saveStatus === 'saving' ? '💾 Guardando...' : saveStatus === 'saved' ? '✅ Guardado' : '💾 Guardar ahora'}
            </button>
          </div>
        </>
      )}
